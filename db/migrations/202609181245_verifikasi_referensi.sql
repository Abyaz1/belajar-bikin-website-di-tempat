...
-- Tipe bersama. Dibuat idempoten; tabel Trust (capture_session, evidence)
-- memakai `vantage` dari sini.
--
-- Urutan: berkas ini (…1245) berjalan sebelum skema jalur tulis Trust
-- (…1300 dst.), karena tabel mereka menunjuk place dan attribute_type.
...
-- ── Hak akses ────────────────────────────────────────────────────────────
-- Migrasi ini berjalan SEBELUM skema jalur tulis Trust (202609181300_*), yang
-- mencabut hak PUBLIC dan memberi app_rw SELECT atas place dan attribute_type.
-- Peran app_rw dibuat di sini dengan pola yang sama (idempoten), supaya tiga
-- tabel aturan bisa dibaca aplikasi (E6, dan nanti E1/E2).
do $$ begin
